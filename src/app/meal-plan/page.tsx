...
export default function MealPlanPage() {
  ...
  const addRecipeToDay = (day: string, recipeId: string) => {
    const updated = plan.map((d) =>
      d.day === day ? { ...d, recipeIds: [...d.recipeIds, recipeId] } : d
    );
    setPlan(updated);
  ...
  const removeRecipeFromDay = (day: string, recipeId: string) => {
    const updated = plan.map((d) => {
      ...
      const idx = d.recipeIds.indexOf(recipeId);
      if (idx === -1) return d;
      const newIds = [...d.recipeIds];
      newIds.splice(idx, 1);
      return { ...d, recipeIds: newIds };
    });
    setPlan(updated);
    saveMealPlan(updated);
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-stone-500 mt-1">
              Pick recipes for the week, then generate your grocery list
            </p>
          </div>
            ...
            {totalMeals > 0 && (
              <>
                ...
                <button
                  ...
                  className="px-4 py-2 border border-stone-200 text-stone-500 rounded-lg text-sm font-medium hover:bg-stone-50 transition-colors"
                >
                  Clear
                </button>
              </>
                ...
                {day.recipeIds.map((id, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        onClick={() => removeRecipeFromDay(day.day, id)}
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-1">
                      {filteredRecipes.slice(0, 10).map((recipe) => (
                        <button
                          key={recipe.id}
                          onClick={() => addRecipeToDay(day.day, recipe.id)}
