    ...
    <AuthGate>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-6">
          <h1 className="text-2xl font-bold text-stone-900 mb-1">Import from URL</h1>
          <p className="text-sm text-stone-500 mb-6">Paste a recipe URL from any website — we&apos;ll strip out the blog story and ads and pull just the recipe.</p>
          ...
          <div className="flex gap-2 mb-6">
            ...
            />
            <button onClick={handleImport} disabled={!url || loading} className="px-5 py-2 bg-amber-500 text-white rounded-lg font-semibold text-sm hover:bg-amber-600 transition-colors disabled:opacity-50 shrink-0">
              {loading ? (
          ...
          {preview && (
            <div className="space-y-4 border-t border-amber-100 pt-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-green-700">Recipe found! Review and edit before saving.</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Source / Chef</label>
              ...
              <div className="pt-4 flex gap-3">
                <button onClick={handleSave} disabled={saving} className="flex-1 bg-amber-500 text-white py-3 rounded-lg font-semibold hover:bg-amber-600 transition-colors disabled:opacity-50">
                  {saving ? "Saving..." : "Save to My Recipes"}
                </button>
                <button onClick={() => { setPreview(null); setUrl(""); }} className="px-6 py-3 border border-amber-200 text-stone-600 rounded-lg font-medium hover:bg-amber-50 transition-colors">
                  Discard
                </button>
              </div>
            </div>
