...
export default function RecipeCard({ recipe }: { recipe: Recipe }) {
  return (
    <div className="group block bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-emerald-100 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
        ...
        <div className="aspect-[4/3] relative overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
          ...
          {isRecipeNew(recipe.created_at) && (
            <div className="absolute top-2 right-2">
              <span className="text-xs font-semibold px-2 py-1 rounded-full bg-emerald-500 text-white shadow">
                New
              </span>
            </div>
          )}
      ...
      <div className="p-4">
        <a href={`/recipe/${recipe.id}`}>
          <h3 className="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-emerald-600 transition-colors leading-tight">
            {recipe.name}
          </h3>
        </a>
        ...
        {recipe.tags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {recipe.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-xs bg-emerald-50 dark:bg-slate-800 text-emerald-700 dark:text-emerald-300 px-2 py-0.5 rounded-full"
              >
                {tag}
              </span>
            ))}
